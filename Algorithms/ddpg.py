...
#                    var = max([var*.9999, var_min]) 
                    ...
                    target_q = critic.predict(next_states, next_state_predicted_actions, target=True).reshape(-1)
                    ...
                    actor.target_model.blend_weights(actor.model, tau=args.tau)
                    critic.target_model.blend_weights(critic.model, tau=args.tau)
